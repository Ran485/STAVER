...
data <- read.csv("/Volumes/Samsung_T5/DIA-QC/results/covid19-2/method_compare/LFQqc_Data.csv", header = T, row.names = 1)
ana_index <- read.csv("/Volumes/Samsung_T5/DIA-QC/results/covid19-2/method_compare/LFQqc_ana.csv", header = T)
ana_index$id <- gsub("#", ".", ana_index$id)
table(ana_index$type)
## 原始数据处理
# data[is.na(data)] == 0
# data = log2(data+1)
res <- group_DEP(data, ana_index, group1 = "Serum", group2 = "Plasma", method = "wilcoxn_test")
