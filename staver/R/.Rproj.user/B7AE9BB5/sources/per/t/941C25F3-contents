...
## --------------- scatter CV plot ------------
QC_data_cv = read.csv("/Users/ranpeng/Desktop/DIA-QC/results/QC_repeat90_highCI_peptides_20221207/Peptide_CV/high_confidence_peptide_cv.csv", check.names = F)
colnames(QC_data_cv)
genes = QC_data_cv
...
genes$CV_threshold = ifelse(genes$`Coefficient of Variation [%]` < 30, "CV% < 30",
                         ifelse(genes$`Coefficient of Variation [%]` < 50, "CV% < 50",
                                ifelse(genes$`Coefficient of Variation [%]` < 100, "CV% < 100",
                                       ifelse(genes$`Coefficient of Variation [%]` < 300, "CV% < 300","CV% >= 300"))))
...
protein_num$CV_thresholds <- factor(protein_num$CV_thresholds, levels=c("CV% < 30", "CV% < 50", "CV% < 100", "CV% < 300", "CV% >= 300"))
## Inside bars
...
p2
# ggsave(p2, file = "/Volumes/Samsung_T5/DIA-QC/results/Figure/TOP3_peptide_num_barplot.pdf", width = 7, height = 5.5)

write.csv(protein_num, "/Users/ranpeng/Desktop/DIA-QC/results/QC_repeat90_highCI_peptides_20221207/Peptide_CV/high_confidence_peptide_cv_count.csv")

## --------------- cumulative CV plot ------------

genes = read.csv("/Volumes/Samsung_T5/DIA-QC/results/QC_repeat90_peptides/TOP1_90QC_data_cv_cumulate.csv", check.names = F)
colnames(genes)
genes$CV_threshold = ifelse(genes$`Coefficient of Variation [%]` < 30, "CV% < 30",
                            ifelse(genes$`Coefficient of Variation [%]` < 50, "CV% < 50",
                                   ifelse(genes$`Coefficient of Variation [%]` < 100, "CV% < 100",
...
p1 = ggplot(genes, aes(x = Abundance_Rank, y = `Coefficient of Variation [%]`)) +
  geom_point(aes(color = CV_threshold), shape=1) +  ## changen the shape of the dot point
  scale_size_continuous(range = c(1,4))+
  scale_color_manual(values = c("#515151", "red", "gray", "#7AA9CE", "#EA686B")) + # ("gray", "#7AA9CE") c( "#EA686B","#7AA9CE","gray"))
  theme_bw(base_size = 12) + theme(legend.position = "right") +
  ...
  geom_hline(yintercept = 100,color = 'gray', linetype="dashed")+
  geom_hline(yintercept = 250,color = 'gray', linetype="dashed")+
  annotate('text', x=20000, y=15, label='Nums = 16673', colour="black",family="Arial",size=6) +
  annotate('text', x=20000, y=40, label='Nums = 3823', colour="black",family="Arial",size=6) +
  annotate('text', x=20000, y=70, label='Nums = 5929', colour="black",family="Arial",size=6) +
  annotate('text', x=20000, y=170, label='Nums = 4271', colour="black",family="Arial",size=6) +
  annotate('text', x=20000, y=260, label='Nums =  4', colour="black",family="Arial",size=6) +
  # xlim(min, max)
  ylim(0, 300) 
