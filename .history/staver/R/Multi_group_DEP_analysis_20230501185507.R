...
# data = data[,-1]
data <- read.csv("/Volumes/T7_Shield/DIANN_Data/merge_data/PXD025752.csv", header = T, row.names = 1, skip = 0)
ana_index <- read.csv("/Volumes/T7_Shield/DIANN_Data/merge_data/RM_info.csv", header = T)
...
write.csv(res, "~/results/clinical_EDA/continuous_clinical/significance_analysis/continuous_clinical_info_DEP_Bad_Good_Circulation_wilcoxn_test_EN.csv")
...
if (TRUE) {
  ...
  ## 数据存储路径
  setwd("/Volumes/T7_Shield/DIANN_Data/merge_data/data/processed_data/DEP/IPX0002924001/DEP_normal_res")
  outpath <- getwd()
}
### 相关卡值
AdjPvalueCutoff <- 1
