...
# -*- encoding: utf-8 -*-
'''
@File     : get_filename_size.py
@Time     : 2023/05/02 00:06:39
@Author   : RanPeng 
    ...
    for root, dirs, files in os.walk(dir):
        for file in files:
            if file.endswith(format):
                ...
                filesize_list.append(
                    round(get_file_size(os.path.join(root, file), SIZE_UNIT.MB), 4)
                )
    # filesize_list = [convert_unit(filesize, 2) for filesize in filesize_list]
    pd.DataFrame(
        zip(filename_list, filesize_list), columns=['filename', 'filesize']
    ).to_csv(os.path.join(dir, 'filename_list.csv'), index=True)
